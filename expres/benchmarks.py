...
def compute(bid, pid, problem, limit, watchlist_dir=None, force=False):
...
def runjob(job):
   return compute(job[0], job[1], job[2], job[3], job[4])

def runjob_force(job):
   return compute(job[0], job[1], job[2], job[3], job[4], force=True)

def eval(bid, pids, limit, watchlist_dir=None, cores=4, force=False):
   probs = problems(bid)
   ...
   jobs = [(bid,pid,problem,limit,watchlist_dir) for pid in pids for problem in probs]
   pool = Pool(cores)
